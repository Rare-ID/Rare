...
- `Rare-Sors/Rare` 的 `main` 分支现在会自动触发：
  - `packages/python/rare-identity-protocol-python/**` 或 `packages/python/rare-agent-sdk-python/**` 变更 -> 自动发布对应 PyPI 包
  - `packages/ts/rare-platform-kit-ts/**` 变更 -> 自动发布 `@rare-id/platform-kit-*`
