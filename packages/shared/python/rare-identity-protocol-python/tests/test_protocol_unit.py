...
@pytest.mark.parametrize("builder", [build_set_name_payload, build_register_payload])
def test_name_payload_builders_validate_name_policy(builder) -> None:
    assert builder(agent_id="agent", name="  Ａｌｉｃｅ　", nonce="n", issued_at=1, expires_at=2) == (
        "rare-name-v1:agent:Alice:n:1:2"
        if builder is build_set_name_payload
        else "rare-register-v1:agent:Alice:n:1:2"
    )

    with pytest.raises(TokenValidationError, match="between 1 and 48"):
        builder(agent_id="agent", name=" ", nonce="n", issued_at=1, expires_at=2)
    with pytest.raises(TokenValidationError, match="reserved"):
        builder(agent_id="agent", name="admin", nonce="n", issued_at=1, expires_at=2)
    with pytest.raises(TokenValidationError, match="control characters"):
        builder(agent_id="agent", name="hello\nworld", nonce="n", issued_at=1, expires_at=2)
