...
IDENTITY_SUB = generate_ed25519_keypair()[1]
    ...
    parsed = parse_rare_jwks(
        {
            "keys": [
                ...
                {"kid": 123, "kty": "OKP", "crv": "Ed25519", "x": public_key_b64},
                "invalid",
            ]
        }
    )
...
def test_verify_identity_attestation_accepts_unknown_claims_for_forward_compat() -> None:
    ...
    assert verified.payload["sub"] == IDENTITY_SUB
...
@pytest.mark.parametrize(
    ("payload_overrides", "error_match"),
    [
        ({"sub": 123}, "identity subject missing"),
        ({"sub": "not-a-pubkey"}, "identity subject must be Ed25519 public key"),
    ],
)
def test_verify_identity_attestation_rejects_invalid_subject(
    payload_overrides: dict[str, object],
    error_match: str,
) -> None:
    token, key_map = _sign_identity_token(payload_overrides=payload_overrides)
...
def test_verify_delegation_token_rejects_invalid_agent_id_for_rare_signer_tokens() -> None:
    rare_private_b64, rare_public_b64 = generate_ed25519_keypair()
    _, session_public_b64 = generate_ed25519_keypair()
    token = sign_jws(
        payload=_delegation_payload(
    ...
    with pytest.raises(TokenValidationError, match="delegation agent_id must be Ed25519 public key"):
        verify_delegation_token(
            token,
            expected_aud="platform",
            required_scope="login",
