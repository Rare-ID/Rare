...
def _require_public_key_b64(value: object, *, missing_message: str, invalid_message: str) -> str:
    if not isinstance(value, str):
        raise TokenValidationError(missing_message)

    try:
        load_public_key(value)
    except TokenValidationError as exc:
        raise TokenValidationError(invalid_message) from exc
...
def verify_identity_attestation(
    ...
    *,
    ...
    _require_public_key_b64(
        payload.get("sub"),
        missing_message="identity subject missing",
        invalid_message="identity subject must be Ed25519 public key",
    )
...
def verify_delegation_token(
    ...
    *,
    ...
    agent_id = _require_public_key_b64(
        payload.get("agent_id"),
        missing_message="delegation agent_id missing",
        invalid_message="delegation agent_id must be Ed25519 public key",
    )
