...
`rare login` 默认从平台 challenge 响应里读取 `aud`，然后把这个值放进协议签名输入。`--aud` 仍然支持，但它现在是可选的严格 pin：如果传入的值和 challenge 返回的 `aud` 不一致，CLI 会拒绝继续。`rare issue-full-attestation --aud <aud>` 保持不变，因为这个命令不访问平台 URL，必须显式知道目标 audience。
